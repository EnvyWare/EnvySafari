...
public class PixelSafariAttribute extends ManagedForgeAttribute<PixelSafariForge> {
    ...
    public void startSafari(String zone) {
        ...
        if (this.manager.getConfig().getSafariSettings().isCacheInventory()) {
            ...
            this.parent.getParent().getInventory().clearContent();
            ...
            for (ConfigItem temporaryItem : temporaryItems) {
                this.parent.getParent().getInventory().add(UtilConfigItem.fromConfigItem(temporaryItem));
            }

            this.parent.getParent().initMenu(this.parent.getParent().inventoryMenu);
        }

        var world = UtilWorld.findWorld(this.manager.getConfig().getWorldName());
